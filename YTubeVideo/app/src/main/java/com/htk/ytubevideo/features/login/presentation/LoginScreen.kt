...
import com.htk.ytubevideo.core.base.onViewModelState
...
internal fun SetupLoginScreen(
    ...
    fun navigateToHome() {
        loginViewModel.redirectToHomeScreen()
    }

    loginViewModel.onViewModelAction { action ->
        when (action) {
...
internal fun SetupLoginScreenBody(loginViewModel: LoginViewModel) {
    Scaffold { innerPadding ->
        loginViewModel.onViewModelState {
            when (this) {
                is LoginState -> {
                    if (showLoading) CustomCircularProgressIndicator() else {
                        ShowScreen(loginViewModel = loginViewModel, innerPadding = innerPadding)
                    }
                }
            }
...
@Composable
fun ShowScreen(loginViewModel: LoginViewModel, innerPadding: PaddingValues) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(
